...
let uvToTorus = (u,v) =>{
  let theta = 2*Math.PI*u;
  let phi = 2*Math.PI*v;
  let r = .5;    //picked an arbitrary radius here, can change
  return [Math.cos(theta)*(1+r*Math.cos(phi)),Math.sin(theta)*(1+r*Math.cos(phi)),r*Math.sin(phi),Math.cos(theta)*Math.cos(phi),Math.sin(theta)*Math.cos(phi),Math.sin(phi)];
 }
